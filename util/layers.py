...
def netVLAD_all_layers(inputs, num_clusters, soft_kernal_name, vlad_kernal_name, assign_weight_initializer=None, 
            cluster_initializer=None, skip_postnorm=False):
    ''' skip_postnorm: Only there for compatibility with mat files. '''
    K = num_clusters
    # D: number of (descriptor) dimensions.
    ...
    # VLAD core.
    C = tf.get_variable(vlad_kernal_name, [1, 1, 1, D, K],
                        initializer=cluster_initializer,
                        dtype=inputs.dtype)
